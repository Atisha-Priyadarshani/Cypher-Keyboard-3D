...
export const Keyboard = forwardRef<KeyboardRefs, KeyboardProps>(
  ({ keycapMaterial, knobColor, ...props }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      // ... (keeping the imperative handle the same)
      plate: plateRef,
      topCase: topCaseRef,
      weight: weightRef,
      screen: screenRef,
    ...
    // Use Zustand store for dynamic theme
    const activeKeycapTheme = useConfiguratorStore((state) => state.activeKeycapTheme);
    const playKeySound = useConfiguratorStore((state) => state.playKeySound);

    const keycapTexture = useTexture(`/${activeKeycapTheme}_uv.png`);
    ...
    return (
      <group {...props} dispose={null} ref={containerRef} onPointerDown={(e) => { e.stopPropagation(); playKeySound(); }}>
